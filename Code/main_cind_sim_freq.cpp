...
int main() {
  ...
  std::vector<double> claims_cind (BB, 0);
  ...
  for (int i=0; i<nobs; i++) {
    ...
    for (int b=0; b<BB; b++) {
      y3cind_mean(i) = y3cind_mean(i) + claims_cind[b];
    }    
    ...
    filenamei = "out_cind_freq/" + filenamei;
  ...
  std::ofstream file_cind_mean("y3cind_mean_freq.txt");
