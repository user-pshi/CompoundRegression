...
# Freq: Poisson + Severity: Gamma
# Need to run DeductibleModels.R to obtain "coef.deductGB2.RDS"
# Need to run FrequencyModels.R to obtain "coef.freq.Poisson.RDS"
...
gam2 <- readRDS(file="coef.freq.Poisson.RDS")
...
# pdf of Poisson
fcount <- function(y){
  ans <- dpois(y,lambda=meanp)
  ans
}
# cdf of Poisson
Fcount <- function(y){
  ans <- ppois(y,lambda=meanp)
  ans
}
...
# Gamma
fit <- glm(yy3~TypeCity+TypeCounty+TypeSchool+TypeTown+TypeVillage+log(Coverage),Gamma(link = "log"), data=datin.clm)
gam3 <- c(coef(fit),summary(fit)$dispersion)
...
loglikJoint1 <- function(parms,fam){
  ...
  # severity|deductible
  u2 <- BiCopHfunc1(pGB2(yy1,mu1,sigma1,alpha11,alpha21),pgamma(yy3,shape=shape,scale=scale),fam[1],rho1,df1)
  # density severity|deductible
  temp1 <- dgamma(yy3,shape=shape,scale=scale)
  temp2 <- BiCopPDF(pGB2(yy1,mu1,sigma1,alpha11,alpha21),pgamma(yy3,shape=shape,scale=scale),fam[1],rho1,df1)
  # (frequency,severity)|deductible
  temp3 <- BiCopHfunc2(u1,u2,fam[2],rho2,df2)-BiCopHfunc2(u1a,u2,fam[2],rho2,df2)
  
  # - log likelihood
  llk <- -sum(log(temp1)) - sum(log(temp2)) - sum(log(temp3+(temp3==0)*1e-12))
  llk
