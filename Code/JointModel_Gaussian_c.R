...
loglikJoint1 <- function(parms,fam){
  ...
  resid <- qnorm((BiCopCDF(u1,u2,fam[2],rho2,df2) - BiCopCDF(u1a,u2,fam[2],rho2,df2))/(u1-u1a+1e-12))
...
zop <- nlminb(ini.joint,loglikJoint,fam=c(1,1),lower =c(rep(-Inf,ncol(cova)),-Inf,0.001,0.001,-0.999,-0.999),upper=c(rep(Inf,ncol(cova)),Inf,Inf,Inf,0.999,0.999),control=list(eval.max=500))
print(zop)
hess<-hessian(loglikJoint,zop$par,fam=c(1,1))
se <-sqrt(diag(solve(hess[1:12,1:12])))


# estimated copula parameter in Table S.5
cbind(c(zop1$par[1],zop$par[11:12]),c(se1,se[11:12]))
