...
VectorXd SimPolicy(double y1, VectorXd cova, VectorXd cova1, VectorXd gam1, VectorXd gam2, VectorXd gam3,
                   VectorXd copDN, VectorXd copDS, double coverage) {
...
VectorXd SimPolicyInd(double y1, VectorXd cova, VectorXd cova1, VectorXd gam1, VectorXd gam2, VectorXd gam3, VectorXd copDN, VectorXd copDS, double coverage) {
...
int main() {
  ...
  MatrixXi tempInt;
  VectorXd y1, gam1, gam2, gam3, copDN, copDS, coverage;
  VectorXi policynum;
  ...
  VectorXd SimDep; SimDep.resize(BB); 
  ...
  for (int rep=0; rep<250; rep++) {
    ...
    for (int iter=600; iter<700; iter++) {
      ...
      SimDep = SimPolicy(500/coverage(442), covatemp, cova1temp, gam1, gam2, gam3, copDN, copDS, coverage(442));
      ...
      //filenamei_dep_freq = "simoutput_dep_freq/" + filenamei_dep_freq;
      std::ofstream filei_dep(filenamei_dep);
      if (filei_dep.is_open()) {
        for (int b=0; b<BB; b++) {
          filei_dep << SimDep(b);
          filei_dep << endl;
        }
      }
      //std::ofstream filei_dep_freq(filenamei_dep_freq);
      //if (filei_dep_freq.is_open()) {
      //  for (int b=0; b<BB; b++) {
      //    filei_dep_freq << SimDep(b+BB);
      //    filei_dep_freq << endl;
      //  }
      //}
    }
    ...
    for (int iter=700; iter<800; iter++) {
    ...
    for (int iter=800; iter<900; iter++) {
    ...
    for (int iter=900; iter<1000; iter++) {
    ...
    for (int iter=1000; iter<1100; iter++) {
    ...
    for (int iter=1100; iter<1200; iter++) {
