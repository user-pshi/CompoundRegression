...
int main() {
  ...
  std::vector<double> pureprem_dep (BB, 0);
  ...
  for (int i=0; i<nobs; i++) {
    ...
    for (int b=0; b<BB; b++) {
      ...
      if (y2dep>0) {
        ...
        paramsdep = {0, mu1(i), sigma1, alpha11, alpha21, copDN(1), copDN(2), y1(i), y2dep, pzero(i), pone(i), meannb(i), size, mu3dep(i), sigma3dep, alpha13dep, alpha23dep, rhoRY, df1, thetaNY, df2};
        F.function = &obj1dep;
        F.params = &paramsdep;
        ...
        for (int j=0; j<y2dep; j++) {
          ...
          x_lo = 0.0;
          x_hi = LARGENUMBER;
          paramsdep = {buff(0,j), mu1(i), sigma1, alpha11, alpha21, copDN(1), copDN(2), y1(i), y2dep, pzero(i), pone(i), meannb(i), size, mu3dep(i), sigma3dep, alpha13dep, alpha23dep, rhoRY, df1, thetaNY, df2};
          F.function = &obj1dep;
          F.params = &paramsdep;
      ...
      for (int j=0; j<y2dep; j++) {
        if (y3(b,j)>coverage(i)*1000000)
          pureprem_dep[b] = pureprem_dep[b] + coverage(i)*1000000; // (Error fixed) This was the error. We should multiply 1000,000 to the coverage.
        else
    ...
    for (int b=0; b<BB; b++) {
      y3dep_mean(i) = y3dep_mean(i) + pureprem_dep[b];
    }    
  ...
  std::ofstream file_dep_mean("y3dep_mean.txt");
