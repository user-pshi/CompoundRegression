...
int main() {
  ...
  std::vector<double> pureprem_ind (BB, 0);
  ...
  for (int i=0; i<nobs; i++) {
    ...
    for (int b=0; b<BB; b++) {
      ...
      if (y2ind>0) {
        ...
        paramsdep = {0, mu1(i), sigma1, alpha11, alpha21, 0, copDN(2), y1(i), y2ind, pzero(i), pone(i), meannb(i), size, mu3ind(i), sigma3ind, alpha13ind, alpha23ind, 0.000000000000000000001, df1, -1, df2};
        F.function = &obj1dep;
        F.params = &paramsdep;
        ...
        for (int j=0; j<y2ind; j++) {
          ...
          x_lo = 0.0;
          x_hi = LARGENUMBER;
          paramsdep = {buff(0,j), mu1(i), sigma1, alpha11, alpha21, 0, copDN(2), y1(i), y2ind, pzero(i), pone(i), meannb(i), size, mu3ind(i), sigma3ind, alpha13ind, alpha23ind, 0.000000000000000000001, df1, -1, df2};
          F.function = &obj1dep;
          F.params = &paramsdep;
      ...
      for (int j=0; j<y2ind; j++)
        if (y3(b,j)>coverage(i)*1000000)
          pureprem_ind[b] = pureprem_ind[b] + coverage(i)*1000000;
        else
    ...
    // std::sort (pureprem_ind.begin(), pureprem_ind.begin()+BB);
    // y3ind_median(i) = pureprem_ind[BB2];

    for (int b=0; b<BB; b++) {
      y3ind_mean(i) = y3ind_mean(i) + pureprem_ind[b];
    }    
    y3ind_mean(i) = y3ind_mean(i) / BB;
    
    cout << "Simulated mean: " << y3ind_mean(i) << endl;
    
    std::string filenamei = std::to_string(policynum(i)) + ".txt";
    filenamei = "out_ind/" + filenamei;
  ...
  std::ofstream file_ind_mean("y3ind_mean.txt");
  ...
  // std::ofstream file_ind_median("y3ind_median.txt");
  // if (file_ind_median.is_open()) {
  //   for (int i=0; i<nobs; i++) {
  //     file_ind_median << y3ind_median(i);
  //     file_ind_median << endl;
  //   }
  // }
  return(0);
}
