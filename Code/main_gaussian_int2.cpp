...
double funcY(double y3, void *params) {
  ...
  // vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::gaussian, 0);
  // vinecopulib::Bicop cop_model_RY(vinecopulib::BicopFamily::gaussian, 0);
  // vinecopulib::Bicop cop_model_NY(vinecopulib::BicopFamily::gaussian, 0);
  MatrixXd tempmat(1,1); 
  tempmat(0,0) = copDNparam(0); vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::gaussian, 0, tempmat);
  tempmat(0,0) = copRYparam(0); vinecopulib::Bicop cop_model_RY(vinecopulib::BicopFamily::gaussian, 0, tempmat);
  tempmat(0,0) = copNYparam(0); vinecopulib::Bicop cop_model_NY(vinecopulib::BicopFamily::gaussian, 0, tempmat);
...
int main() {
  ...
  VectorXd mu3dep, mu3ind, mu3cind, tempvec, mu3Gaussian;
  int k1, k, nobs;
  ...
  double sigma3Gaussian, alpha13Gaussian, alpha23Gaussian, copDNGaussian, rho1Gaussian, rho2Gaussian;
  double sigma3cind, alpha13cind, alpha23cind;
  double uR, uYind, uYdep, pp0, pp1, tempval1, tempval2, y2dep, y2ind, y2cind;
  VectorXd y1, gam1, gam2, gam3, copDN, copDS, copDScind, copGaussian, coverage, deduct;
  VectorXi policynum;
  VectorXd y3dep_mean;
  gsl_function F;
  ...
  mu3Gaussian = cova * copGaussian.head(k);
  sigma3Gaussian = copGaussian(k);
  alpha13Gaussian = copGaussian(k+1);
  alpha23Gaussian = copGaussian(k+2);
  copDNGaussian = copGaussian(k+3);
  rho1Gaussian = copGaussian(k+4);
  rho2Gaussian = copGaussian(k+5);
  
  std::default_random_engine generator;
  std::uniform_real_distribution<double> uni_dist(0.0,1.0);
  tempmat(0,0) = copDNGaussian; 
  vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::gaussian, 0, tempmat);
  ...
  for (int i=0; i<nobs; i++) {
    ...
    paramsdep = {coverage(i), deduct(i), mu1(i), sigma1, alpha11, alpha21, pzero(i), pone(i), meannb(i), size, mu3Gaussian(i), sigma3Gaussian, alpha13Gaussian, alpha23Gaussian, copDNGaussian, copDN(2), rho1Gaussian, df1, rho2Gaussian, df2};
    F.params = &paramsdep;
    ...
    for (int n=1; n<MAXN; n++) {
      double y2 = (double)n;
      Vector2d copDNparam; copDNparam << copDNGaussian, copDN(2);
      Vector2d copRYparam; copRYparam << rho1Gaussian, df1;
      Vector2d copNYparam; copNYparam << rho2Gaussian, df2;
      // vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::gaussian, 0);
      // vinecopulib::Bicop cop_model_RY(vinecopulib::BicopFamily::gaussian, 0);
      // vinecopulib::Bicop cop_model_NY(vinecopulib::BicopFamily::gaussian, 0);
      tempmat(0,0) = copDNparam(0); vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::gaussian, 0, tempmat);
  ...
  std::ofstream file_dep_mean("mean_gaussian_int2.txt");
