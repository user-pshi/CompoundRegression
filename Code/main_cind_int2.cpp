...
double funcY(double y3, void *params) {
  ...
  vinecopulib::Bicop cop_model_NY(vinecopulib::BicopFamily::indep, 0);
...
int main() {
  ...
  for (int i=0; i<nobs; i++) {
    ...
    paramsdep = {coverage(i), deduct(i), mu1(i), sigma1, alpha11, alpha21, pzero(i), pone(i), meannb(i), size, mu3cind(i), sigma3cind, alpha13cind, alpha23cind, copDN(1), copDN(2), rhoRY, df1, -1.0, df2};
    F.params = &paramsdep;
    ...
    for (int n=1; n<MAXN; n++) {
      ...
      // severity|deductible
      utemp(0,1) = pGB2(coverage(i)*1000000,mu3cind(i),sigma3cind,alpha13cind,alpha23cind);
      tempvec = cop_model_RY.hfunc1(utemp);
      u2 = tempvec(0);
      utemp1(0,0) = u1; utemp1(0,1) = u2; utemp2(0,0) = u1a; utemp2(0,1) = u2;
    ...
    y3cind_mean(i) += AdjVal;
    
    // gsl_integration_qags (&F, coverage(i)*1000000, coverage(i)*1000000*10000, 0, 1e-7, 1000, w, &result, &error); 
    cout << "Result: " << y3cind_mean(i) << ", Adjustment: " << AdjVal << endl;
  }
  
  std::ofstream file_cind_mean("mean_cind_int2.txt");
