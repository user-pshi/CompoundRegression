...
int main() {
  ...
  std::vector<double> claims_ind (BB, 0);
  ...
  for (int i=0; i<nobs; i++) {
    ...
    for (int b=0; b<BB; b++) {
      y3ind_mean(i) = y3ind_mean(i) + claims_ind[b];
    }    
    ...
    filenamei = "out_ind_freq/" + filenamei;
  ...
  std::ofstream file_ind_mean("y3ind_mean_freq.txt");
