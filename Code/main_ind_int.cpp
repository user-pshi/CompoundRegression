...
//******************************************************************************************
// main_ind_int.cpp: Predict aggregate claims using the integral approach for the independence model.
//******************************************************************************************
...
double funcY(double y3, void *params) {
  ...
  vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::indep, 0);
  ...
  for (int n=1; n<MAXN; n++) {
    ...
    tempvec = cop_model_DN.hfunc1(utemp); // independence copula. Abuse DN copula.
    u2 = tempvec(0);
    utemp1(0,0) = u1; utemp1(0,1) = u2; utemp2(0,0) = u1a; utemp2(0,1) = u2;
    tempvec1 = cop_model_DN.hfunc2(utemp1); // independence copula. Abuse DN copula.
    tempvec2 = cop_model_DN.hfunc2(utemp2); // independence copula. Abuse DN copula.
...
int main() {
  ...
  VectorXd mu1, beta, gamma0, gamma1, pzero, pone, meannb, tempvec0, tempvec1, gam3dep;
  VectorXd mu3dep, mu3ind, tempvec;
  int k1, k, nobs;
  double rhoRY, thetaNY, fam1, fam2, df1, df2;
  double sigma1, alpha11, alpha21, size;
  double sigma3dep, alpha13dep, alpha23dep;
  double sigma3ind, alpha13ind, alpha23ind;
  double pp0, pp1, tempval1, tempval2;
  VectorXd y1, gam1, gam2, gam3, copDN, copDS, coverage, deduct;
  VectorXi policynum;
  VectorXd y3ind_mean;
  gsl_function F;
  ...
  temp = readMatrix("dat_y1.txt"); y1 = temp.col(0);
  temp = readMatrix("dat_coverage.txt"); coverage = temp.col(0);
  temp = readMatrix("dat_deduct.txt"); deduct = temp.col(0);
  temp = readMatrix("coef.deductGB2.txt"); gam1 = temp.col(0);
  temp = readMatrix("coef.freq.txt"); gam2 = temp.col(0);
  temp = readMatrix("coef.sev.txt"); gam3 = temp.col(0);
  temp = readMatrix("cop.DN.txt"); copDN = temp.col(0);
  temp = readMatrix("cop.DS.txt"); copDS = temp.col(0);
  cova = readMatrix("dat_cova.txt");
  cova1 = readMatrix("dat_cova1.txt");
  tempInt = readMatrixInt("dat_policynum.txt"); policynum = tempInt.col(0);
  k1 = cova1.cols();
  k = cova.cols();
  nobs = cova.rows();
  ...
  for (int i=0; i<nobs; i++) {
    ...
    paramsdep = {coverage(i), deduct(i), mu1(i), sigma1, alpha11, alpha21, pzero(i), pone(i), meannb(i), size, mu3ind(i), sigma3ind, alpha13ind, alpha23ind, 0.0, copDN(2), 0.00000000000000000001, df1, -1.0, df2};
    F.params = &paramsdep;
    ...
    for (int n=1; n<MAXN; n++) {
      double y2 = (double)n;
      Vector2d copDNparam; copDNparam << 0.0, copDN(2);
      Vector2d copRYparam; copRYparam << 0.00000000000000000001, df1;
      Vector2d copNYparam; copNYparam << -1.0, df2;
      vinecopulib::Bicop cop_model_DN(vinecopulib::BicopFamily::indep, 0);
      vinecopulib::Bicop cop_model_RY(vinecopulib::BicopFamily::frank, 0, copRYparam.head(1));
      vinecopulib::Bicop cop_model_NY(vinecopulib::BicopFamily::gumbel, 90, -copNYparam.head(1));
      Matrix<double, 1, 2> utemp, utemp1, utemp2;
      ...
      // severity|deductible
      utemp(0,1) = pGB2(coverage(i)*1000000,mu3ind(i),sigma3ind,alpha13ind,alpha23ind);
      // tempvec = cop_model_RY.hfunc1(utemp);
      tempvec = cop_model_DN.hfunc1(utemp); // independence copula. abusing DN copula since it is independence copula.
      u2 = tempvec(0);
      utemp1(0,0) = u1; utemp1(0,1) = u2; utemp2(0,0) = u1a; utemp2(0,1) = u2;
      // tempvec1 = cop_model_NY.cdf(utemp1);
      // tempvec2 = cop_model_NY.cdf(utemp2);
      tempvec1 = cop_model_DN.cdf(utemp1); // independence copula. abusing DN copula since it is independence copula.
      tempvec2 = cop_model_DN.cdf(utemp2); // independence copula. abusing DN copula since it is independence copula.
    ...
    cout << "Result: " << y3ind_mean(i) << ", Adjustment: " << AdjVal << endl;
  }
  
  std::ofstream file_ind_mean("mean_ind_int.txt");
  if (file_ind_mean.is_open()) {
    for (int i=0; i<nobs; i++) {
      file_ind_mean << y3ind_mean(i);
      file_ind_mean << endl;
    }
  }
