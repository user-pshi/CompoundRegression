...
#define BB 5000 // number of replicates.
...
int main() {
  ...
  VectorXd y3cind_mean;
  // double pureprem_dep[BB], pureprem_ind[BB], pureprem_cind[BB];
  std::vector<double> pureprem_cind (BB, 0);
  ...
  y3cind_mean.resize(nobs);
  // y3cind_median.resize(nobs);
  ...
  for (int i=0; i<nobs; i++) {
    ...
    for (int b=0; b<BB; b++) {
      ...
      // cout << "y2dep: " << y2dep << endl;
      if (y2cind>0) {
        
        // Figure out the maximum of the uniform distribution that we can accomodate.
        paramsdep = {0, mu1(i), sigma1, alpha11, alpha21, copDN(1), copDN(2), y1(i), y2cind, pzero(i), pone(i), meannb(i), size, mu3cind(i), sigma3cind, alpha13cind, alpha23cind, rhoRY, df1, -1, df2};
        F.function = &obj1dep;
        F.params = &paramsdep;
        double MaxU = F.function(coverage(i)*1000000,&paramsdep);
        for (int j=0; j<y2cind; j++) { buff(0,j) = uni_dist(generator); if(buff(0,j)>MaxU) {buff(0,j)=MaxU;}}
        for (int j=0; j<y2cind; j++) {
          iter = 0; max_iter = 100;
          r = 0.1; 
          x_lo = 0.0;
          x_hi = LARGENUMBER;
          paramsdep = {buff(0,j), mu1(i), sigma1, alpha11, alpha21, copDN(1), copDN(2), y1(i), y2cind, pzero(i), pone(i), meannb(i), size, mu3cind(i), sigma3cind, alpha13cind, alpha23cind, rhoRY, df1, -1, df2};
          F.function = &obj1dep;
          F.params = &paramsdep;
      ...
      pureprem_cind[b] = 0;
      for (int j=0; j<y2cind; j++)
        if (y3(b,j)>coverage(i)*1000000)
          pureprem_cind[b] = pureprem_cind[b] + coverage(i)*1000000;
        else
          pureprem_cind[b] = pureprem_cind[b] + y3(b,j);
      // file_sim_dep << y3[0][0];
      // for (int j=1; j<NN; j++)
      //   file_sim_dep << "," << y3[0][j];
      // file_sim_dep << endl;
    ...
    // std::sort (pureprem_cind.begin(), pureprem_cind.begin()+BB);
    // y3cind_median(i) = pureprem_cind[BB2];

    for (int b=0; b<BB; b++) {
      y3cind_mean(i) = y3cind_mean(i) + pureprem_cind[b];
    }    
    y3cind_mean(i) = y3cind_mean(i) / BB;
    
    cout << "Simulated mean: " << y3cind_mean(i) << endl;
    
    std::string filenamei = std::to_string(policynum(i)) + ".txt";
    filenamei = "out_cind/" + filenamei;
  ...
  std::ofstream file_cind_mean("y3cind_mean.txt");
  if (file_cind_mean.is_open()) {
    for (int i=0; i<nobs; i++) {
      file_cind_mean << y3cind_mean(i);
      file_cind_mean << endl;
    }
  }
  
  // std::ofstream file_cind_median("y3cind_median.txt");
  // if (file_cind_median.is_open()) {
  //   for (int i=0; i<nobs; i++) {
  //     file_cind_median << y3cind_median(i);
  //     file_cind_median << endl;
  //   }
  // }
  return(0);
}
